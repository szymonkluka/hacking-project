...
exports.deletePhoto = async (req, res) => {
  try {
    ...
    const photo = await Photo.findOneAndDelete({ _id: photoId }); // Find and delete the selected photo
    if (!photo) {
      return res.status(404).json({ message: 'Not found' });
    }
...
exports.removeVote = async (req, res) => {
  try {
    ...
    if (!voter.votes.includes(photoId)) {
      return res.status(500).json({ message: 'You have not voted for this photo yet.' }); // The user has not voted for this photo
    }

    voter.votes = voter.votes.filter((vote) => vote.toString() !== photoId.toString()); // Remove the photo ID from the voter's votes array
    await voter.save(); // Save the Voter document

    photo.votes--; // Decrease the number of votes for the photo
    await photo.save();

    res.send({ message: 'Vote removed successfully.' });
  } catch (err) {
    res.status(500).json(err);
  }
};
